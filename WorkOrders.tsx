        ...
        <div className="flex gap-2 mb-4">
          {(["gst", "tax", "retail"] as const).map(t => (
            <button key={t} onClick={() => setInvoiceType(t)}
              className={`px-3 py-1.5 rounded-md text-sm border ${invoiceType === t ? "bg-muted/30 text-muted-foreground uppercase tracking-wider text-xs border-primary" : "bg-card border-input"}`}>
