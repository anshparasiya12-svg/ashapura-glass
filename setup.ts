  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-black text-foreground tracking-tight">Product Master</h1>
      ...
      {showForm && (
        <div className="bg-card/50 backdrop-blur-xl rounded-2xl border border-border/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-5 mb-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Product Name" className="glass-input" />
            <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="glass-input">
              {CATEGORIES.map((c) => <option key={c}>{c}</option>)}
            </select>
            <select value={form.thickness_mm} onChange={(e) => setForm({ ...form, thickness_mm: Number(e.target.value) })} className="glass-input">
              {THICKNESSES.map((t) => <option key={t} value={t}>{t} mm</option>)}
            </select>
            <input value={form.hsn_code} onChange={(e) => setForm({ ...form, hsn_code: e.target.value })} placeholder="HSN Code" className="glass-input" />
            <select value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })} className="glass-input">
              {["sqft", "nos", "mtr", "kg", "box", "set", "pcs"].map((u) => <option key={u}>{u}</option>)}
            </select>
            <input type="number" value={form.rate} onChange={(e) => setForm({ ...form, rate: Number(e.target.value) })} placeholder="Rate" className="glass-input" />
            <select value={form.gst_percent} onChange={(e) => setForm({ ...form, gst_percent: Number(e.target.value) })} className="glass-input">
              {[0, 5, 12, 18, 28].map((r) => <option key={r} value={r}>{r}%</option>)}
            </select>
            <input type="number" value={form.stock} onChange={(e) => setForm({ ...form, stock: Number(e.target.value) })} placeholder="Stock" className="glass-input" />
            <input type="number" value={form.low_stock_threshold} onChange={(e) => setForm({ ...form, low_stock_threshold: Number(e.target.value) })} placeholder="Low stock alert" className="glass-input" />
            <div className="flex gap-2 col-span-2">
              <button onClick={save} className="bg-muted/30 text-muted-foreground uppercase tracking-wider text-xs px-3 py-2 rounded-md text-sm flex-1">{editingId ? "Update" : "Add"}</button>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              return (
                <tr key={p.id} className="border-t border-border/50 hover:bg-muted/30 transition">
...
const StockCard = ({ label, value, color }: { label: string; value: number; color: string }) => (
  <div className="bg-card/50 backdrop-blur-xl rounded-2xl border border-border/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-4 flex items-center gap-3">
