  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div className="bg-card/50 backdrop-blur-xl rounded-2xl border border-border/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-5">
        ...
        <div className="bg-card/50 backdrop-blur-xl rounded-2xl border border-border/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-5">
          ...
          <div className="flex justify-between font-bold border-t border-border/50 hover:bg-muted/30 transition pt-2 mt-2"><span>Balance</span><span className="text-destructive">₹{balance.toFixed(2)}</span></div>
        </div>
      </div>

      <div className="bg-card/50 backdrop-blur-xl rounded-2xl border border-border/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-5 mb-4 overflow-x-auto">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map((it, idx) => (
              <tr key={it.id} className="border-t border-border/50 hover:bg-muted/30 transition">
        ...
        <div className="mt-4 flex justify-end">
          <div className="w-72 text-sm space-y-1">
            ...
            <div className="flex justify-between font-bold border-t border-border/50 hover:bg-muted/30 transition pt-2 mt-2 text-base"><span>Net Amount</span><span>₹{net.toFixed(2)}</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card/50 backdrop-blur-xl rounded-2xl border border-border/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-5">
        ...
        {showPay && (
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-4 p-3 bg-muted/30 rounded-md">
            ...
            <button onClick={addPayment} className="bg-muted/30 text-muted-foreground uppercase tracking-wider text-xs rounded-md text-sm">Save</button>
          </div>
        )}
        {payments.length === 0 ? (
